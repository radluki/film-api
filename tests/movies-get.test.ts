...
describe("/movies GET - response validation", () => {
  ...
  function expectRuntimesInBounds(list: any, min: number, max: number) {
    ...
    const runtimesOutOfBound = runtimes.filter(
      (runtime) => runtime < min || runtime > max,
    );
    expect(runtimesOutOfBound).toEqual([]);
  }

  function expectMoviesToHaveSpecifiedGenres(
    movies: any[],
    specifiedGenres: string[],
  ) {
    const invalidMovies = movies.filter(lacksSpecifiedGenre);
    const mappedInvalidMovies = invalidMovies.map((movie) => {
  ...
  it("?duration=115&genres=Animation - should return valid movies", () => {
    return request(url)
      .get("/movies?duration=115&genres=Animation")
      .expect(200)
      .expect("Content-Type", /json/)
      .expect((res) => {
  ...
  it("?genres=Animation - should return Animation movies", () => {
    return request(url)
      .get("/movies?genres=Animation")
      .expect(200)
      .expect("Content-Type", /json/)
      .expect((res) => {
        expectArrayOfMovies(res.body);
        const moviesShorterThan105 = res.body.filter(
          (movie) => movie.runtime < 105,
        );
  ...
  function expectMoviesInOrderOfNumberOfGenreMatches(
    movies: any[],
    genres: string[],
  ) {
    const moviesWithNumberOfGenreMatches = movies.map((movie) => {
      const numberOfGenreMatches = movie.genres.filter((genre) =>
        genres.includes(genre),
      ).length;
      return { title: movie.title, numberOfGenreMatches };
    });
    const sortedMovies = [...moviesWithNumberOfGenreMatches].sort(
      (a, b) => b.numberOfGenreMatches - a.numberOfGenreMatches,
    );

    expect(moviesWithNumberOfGenreMatches.length).toBeGreaterThan(0);
    const lastNumberOfMatches =
      moviesWithNumberOfGenreMatches[moviesWithNumberOfGenreMatches.length - 1]
        .numberOfGenreMatches;
    const firstNumberOfMatches =
      moviesWithNumberOfGenreMatches[0].numberOfGenreMatches;
  ...
  it("/movies?genres=Animation,Comedy,Thriller - should return valid movies", () => {
    return request(url)
      .get("/movies?genres=Animation,Comedy,Thriller")
      .expect(200)
      .expect("Content-Type", /json/)
      .expect((res) => {
        expectArrayOfMovies(res.body);
  ...
  it("/movies?genres=Animation,Comedy,Thriller&duration=120 - should return valid movies", () => {
    return request(url)
      .get("/movies?genres=Animation,Comedy,Thriller&duration=120")
      .expect(200)
      .expect("Content-Type", /json/)
      .expect((res) => {
        expectArrayOfMovies(res.body);
